...
func (s *Scans) Report(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write(data) //nolint:errcheck,gosec // best-effort write after headers already sent; when format=="html" this is generated by internal/reporter's HTMLReporter using html/template, which auto-escapes all interpolated finding/target data, so XSS via reflected scan content is not possible here
}
