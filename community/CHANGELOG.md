...
### Fixed
- CITADEL WORM evidence emission (`internal/citadel/connector.go`) was silently failing on every call — it posted to `/api/v1/events`, a route that has never existed on CITADEL. Now posts to `POST /api/v1/worm/emit`, restoring the audit trail for `community.post.published` and `community.user.deleted` events.
