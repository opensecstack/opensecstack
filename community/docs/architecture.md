...
- **Full-text search** — Meilisearch-backed when configured (see [README.md](../README.md) and [docs/api.md](api.md)), with a PostgreSQL `tsvector` `GENERATED ALWAYS AS STORED` column as the fallback when Meilisearch isn't set up. No Elasticsearch dependency either way.
