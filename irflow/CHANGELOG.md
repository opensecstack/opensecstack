...
First production release of IRFlow — first version cut through a real
release pipeline. The engine covers the full incident lifecycle with
enforced governance (including a two-person propose/approve action
flow), webhook ingestion, playbook automation, JWT-based API access,
sinauth SSO, Prometheus observability, and a real integration test
suite.
