...
def create_app(config_class: type = Config) -> Flask:
    ...
    if not app.config.get("JWT_SECRET") and not app.config.get("DEBUG"):
        raise RuntimeError("NIS2_JWT_SECRET must be set in production. " "Generate one with: openssl rand -hex 32")

    if app.config.get("SECRET_KEY") == "dev-secret-key-do-not-use-in-production" and not app.config.get("DEBUG"):
        raise RuntimeError(
