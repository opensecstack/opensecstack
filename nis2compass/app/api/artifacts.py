...
from ..models import Artifact, Assessment, Control
...
def upload_artifact(assessment_id):
    ...
    if art_type not in VALID_TYPES:
        return (
            jsonify({"error": f'type must be one of: {", ".join(sorted(VALID_TYPES))}', "code": "INVALID_INPUT"}),
            400,
        )
...
def delete_artifact(artifact_id):
    ...
    if file_path and os.path.isfile(file_path):
        ...
        if not real_path.startswith(upload_dir + os.sep) and real_path != upload_dir:
            current_app.logger.error("artifact file_path %r is outside UPLOAD_DIR — deletion blocked", file_path)
        else:
            try:
                os.remove(real_path)
