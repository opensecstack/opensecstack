...
from ..models import Artifact, Assessment, Control, ControlTemplate, Organisation
from ..notifications import notify_assessment_status_change
from ..reporters import generate_json_report, generate_nca_report, generate_sarif_report
...
def _check_report_rate_limit(actor: str) -> bool:
    ...
    if rc is not None:
        ...
        try:
            ...
            current_app.logger.warning("report rate limit: Redis error, falling back to in-memory: %s", exc)
    ...
    with _report_rate_limit_lock:
        ...
        if len(_report_rate_limit) > 1000:
            stale_actors = [k for k, v in _report_rate_limit.items() if not v or v[-1] <= window_start]
...
def _create_controls_from_templates(assessment_id, db_session, framework="nis2"):
    ...
    existing_refs = {
        row.measure_ref
        for row in db_session.query(Control.measure_ref).filter(Control.assessment_id == assessment_id).all()
    }
...
def delete_assessment(assessment_id):
    ...
    artifact_paths = [a.file_path for a in db.session.query(Artifact).filter_by(assessment_id=assessment.id).all()]
    ...
    for path in artifact_paths:
        ...
        if not real_path.startswith(upload_dir + os.sep) and real_path != upload_dir:
            current_app.logger.warning("artifact file_path %r is outside UPLOAD_DIR — deletion skipped", path)
            continue
        try:
            os.remove(real_path)
...
def _generate_pdf(assessment, org, controls, templates):
    ...
    for row_idx, status in status_row_map:
        ...
        ctrl_style_cmds.append(("TEXTCOLOR", (4, row_idx), (4, row_idx), status_colour))
    ...
    footer_text = f"Generated by NIS2 Compass  |  " f'{generated_at.strftime("%Y-%m-%d %H:%M:%S UTC")}'
...
def generate_report(assessment_id):
    ...
    controls = (
        db.session.query(Control).filter(Control.assessment_id == assessment_id).order_by(Control.measure_ref).all()
    )
    ...
    # Default: PDF
    templates = db.session.query(ControlTemplate).order_by(ControlTemplate.measure_ref).all()
