...
from ..models import Artifact, Assessment, ComplianceSnapshot, Control
...
def lock_assessment(assessment_id):
    ...
    write_audit(
        ...
        "assessment",
        assessment.id,
        metadata={"reason": assessment.lock_reason},
        risk_class="WARNING",
    )
    db.session.commit()
...
def sign_artifact(artifact_id):
    ...
    write_audit(
        ...
        "artifact",
        artifact.id,
        metadata={"file_hash": artifact.hash},
        risk_class="WARNING",
    )
    db.session.commit()
