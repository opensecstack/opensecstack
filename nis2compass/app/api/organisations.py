...
def create_organisation():
    ...
    if size not in VALID_SIZES:
        return (
            jsonify({"error": f'size must be one of: {", ".join(sorted(VALID_SIZES))}', "code": "INVALID_INPUT"}),
            400,
        )
    ...
    if entity_type not in VALID_ENTITY_TYPES:
        return (
            jsonify(
                {
                    "error": f'entity_type must be one of: {", ".join(sorted(VALID_ENTITY_TYPES))}',
                    "code": "INVALID_INPUT",
                }
            ),
            400,
    ...
    try:
        ...
        if hasattr(e, "orig") and e.orig is not None and getattr(e.orig, "pgcode", None) == "23505":
            return (
                jsonify(
                    {"error": "An organisation with this name already exists for your account", "code": "CONFLICT"}
                ),
                409,
            )
        raise
...
def update_organisation(org_id):
    ...
    if "registration_number" in data:
        ...
        if reg_num is not None and len(reg_num) > 100:
            return (
                jsonify({"error": "registration_number must not exceed 100 characters", "code": "INVALID_INPUT"}),
                400,
            )
