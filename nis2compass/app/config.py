...
def _build_database_uri() -> str:
    ...
    if not db_password:
        _logger.warning("NIS2_DB_PASSWORD not set — refusing to start with default credentials")
        raise RuntimeError(
            "NIS2_DB_PASSWORD (or NIS2_DB_URL) must be set; " "refusing to start with no database password."
        )
    ...
    return f"postgresql+psycopg2://{db_user}:{encoded_password}" f"@{db_host}:{db_port}/{db_name}"
...
class Config:
    ...
    API_KEYS = [k.strip() for k in os.getenv("NIS2_API_KEYS", "").split(",") if k.strip()]
    ...
    if WEBHOOK_URL:
        ...
        if _parsed.scheme not in ("http", "https"):
            raise ValueError(
                f"NIS2_WEBHOOK_URL has an invalid scheme {_parsed.scheme!r}; " "only http and https are permitted."
            )
        _hostname = _parsed.hostname or ""
        _BLOCKED_HOSTNAMES = {"localhost", "127.0.0.1", "::1"}
        if _hostname in _BLOCKED_HOSTNAMES:
            raise ValueError(f"NIS2_WEBHOOK_URL hostname {_hostname!r} is not allowed (loopback).")
        try:
    ...
    CORS_ORIGINS = (
        os.getenv("NIS2_CORS_ORIGINS", "*").split(",")
        if os.getenv("NIS2_CORS_ORIGINS")
        else (["*"] if os.getenv("NIS2_ENV", "production") == "development" else [])
    )
