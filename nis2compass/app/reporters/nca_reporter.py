...
def generate_nca_report(assessment: Any, controls: list[Any], organisation: Any) -> bytes:
    ...
    root = ET.Element('NCAReport', attrib={
        'version': '1.0',
        'generated_at': now,
    ...
    # ── Assessment element ────────────────────────────────────────────────────
    assessment_el = ET.SubElement(root, 'Assessment', attrib={
        ...
        'period_end': _isoformat(getattr(assessment, 'completed_at', None) or getattr(assessment, 'due_date', None)),
        'compliance_score': str(float(assessment.compliance_score)) if assessment.compliance_score is not None else '',
    })
    ...
    for c in controls:
        ...
        ET.SubElement(controls_el, 'Control', attrib={
    ...
    if gap_report and isinstance(gap_report, dict):
        for gap in gap_report.get('gaps', []):
            ET.SubElement(gaps_el, 'Gap', attrib={
