...
| `CITADEL_API_URL` | No | — (empty) | Base URL of the CITADEL governance service, e.g. `http://citadel-api:8099`. Enables both audit forwarding (`POST /api/v1/worm/emit`, fire-and-forget) and MARSHAL governance evaluation (`POST /api/v1/marshal/evaluate`, blocking) for control status updates, artifact signing, and assessment lock/unlock. If unset, both features are no-ops and behave exactly as before this integration existed. See [architecture.md](architecture.md#external-citadel-forwarding-and-governance-evaluation). |
| `CITADEL_API_KEY` | No | — (empty) | Bearer token sent as `Authorization: Bearer <key>` on requests to CITADEL. Only meaningful when `CITADEL_API_URL` is also set. |
