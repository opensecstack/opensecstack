...
import { Shield, Cpu, Network, Activity } from 'lucide-react'
...
const FEATURES = [
  {
    icon: Cpu,
    title: 'XDP/eBPF in-kernel filtering',
    body: 'Drop malicious traffic at line rate directly in the kernel with XDP/eBPF, before it ever reaches your application stack.',
  },
  {
    icon: Network,
  ...
  {
    icon: Activity,
    title: 'Live mitigations & rules',
    body: 'Author filtering rules and watch active mitigations in real time, with live metrics on dropped and forwarded traffic.',
  },
]
...
export default function Landing(): JSX.Element {
  ...
  useEffect(() => {
    if (isAuthenticated()) navigate('/rules', { replace: true })
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-semibold">OpenScrub</span>
          </div>
          <button
            onClick={handleLogin}
            disabled={pending}
