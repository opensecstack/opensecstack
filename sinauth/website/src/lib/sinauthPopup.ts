...
export async function loginWithPopup(sinauthURL: string, clientId: string, siteURL?: string): Promise<PopupUser> {
  ...
  const popup = window.open(
    `${popupBase}/oauth/login?returnUrl=${encodeURIComponent(returnUrl)}`,
    'sinauth_popup',
  ...
  return new Promise<PopupUser>((resolve, reject) => {
    ...
    const handler = async (event: MessageEvent) => {
      ...
      try {
        const res = await fetch(`${sinauthURL}/oauth/token`, {
          ...
          body: new URLSearchParams({ grant_type: 'authorization_code', code: event.data.code,
    ...
    const poll = setInterval(() => {
      if (popup.closed) { clearInterval(poll); clearTimeout(timeout);
        window.removeEventListener('message', handler); reject(new Error('Popup closed')) }
    }, 500)
  })
}
