// Package corpus loads labelled phishing samples and evaluates the
// scanner's precision/recall against them. Mirrors the prompt-corpus
// shape so contributors can copy the workflow.
package corpus
...
type Sample struct {
	...
	Kind     string   `json:"kind"`     // url | email | html
	Source   string   `json:"source"`
	Tags     []string `json:"tags,omitempty"`
	Notes    string   `json:"notes,omitempty"`
}

// Misclassified records an unexpected verdict.
...
// Report mirrors prompt/corpus.Report.
type Report struct {
	Total         int
...
func parse(r io.Reader) ([]Sample, error) {
	...
	for scanner.Scan() {
		...
		if s.ID == "" || s.Text == "" || s.Expected == "" {
			return nil, fmt.Errorf("line %d: id/text/expected required", line)
		}
		switch s.Expected {
...
// Evaluate runs each sample through scanner and tallies metrics.
func Evaluate(samples []Sample, scanner *phishing.Scanner) Report {
	...
	for _, s := range samples {
		...
		kind := phishing.Kind(s.Kind)
		result, err := scanner.Scan(s.Text, kind)
		if err != nil {
		...
		if actual != s.Expected {
			rep.Misclassified = append(rep.Misclassified, Misclassified{
				...
				Score: result.Confidence, Snippet: snippet(s.Text),
			})
		}
	}
	var macroSum, macroN float64
	for _, v := range verdicts {
...
func (r Report) String() string {
	...
	fmt.Fprintf(&b, "Phishing corpus: %d samples (CLEAN=%d, SUSPICIOUS=%d, BLOCKED=%d)\n",
